...
export class CooldownVo {
  static from(skill: Skill): CooldownVo | undefined {
    if (!skill.cooldown) return undefined;
    return new CooldownVo(skill.cooldown);
  }
  ...
  constructor(
    ...
    if (Number.isNaN(lastUsedAt)) throw new Error(`lastUsedAt must not be NaN`);
  }
