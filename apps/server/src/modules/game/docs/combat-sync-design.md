...
> 狀態：設計定案。瞬發與詠唱路徑均已實作（`castBegin` / 進度條 / 到點結算 / 移動打斷）。
> 尚未實作：死亡與斷線打斷（`died` / `disconnected`）、per-skill 冷卻、體感優化、時鐘完整版——見第 7 節現況。
...
實作上（已定案為「移動優先」，非拒絕移動）：`World.setMoveTarget` 詠唱中會清除詠唱狀態並回傳 `castCancelled { reason: 'moved' }`，由 gateway 廣播。Client 端配合：詠唱中點擊一律送 `move`（即玩家點地圖 = 主動取消詠唱）。

尚未實作的打斷來源：死亡（`died`）、斷線（`disconnected`）——目前 hp 歸 0 無死亡處理，`removePlayer` 也不會廣播取消，其他 client 的進度條會跑滿才消失。
...
> 以下型別均已實作於 `protocol.ts`（含 `castBegin` / `castCancel`）。欄位沿用 combat-design.md 的 `DamageKind` / `DamageElement` / `Multiplier`。
...
| 位置 | 改動 | 狀態 |
|------|------|------|
| `packages/shared/src/protocol.ts` | `attack` 意圖/結果、`castBegin` / `castCancel` 事件與 payload 型別。共用型別 = 兩端編譯期就對齊，不會歪 | ✅ |
| `game.gateway.ts` | `@SubscribeMessage('attack')`；`castBegin` / `castCancel` 廣播（request-born 由 handler 回傳、tick-born 走 `onEvent`，統一過 `broadcast` switch） | ✅ |
| `GameService` / `World` | 詠唱狀態（`Player.casting` = `CastingVo`、`castTargets`）、tick 內檢查詠唱完成、移動時取消詠唱 | ✅（冷卻仍為全域 `ATTACK_COOLDOWN_MS`，per-skill 未做） |
...
| client `GameSession.ts` + domain | `castBegin` / `castCancel` 監聽 → `ActiveCastTracker`（`active-casts.ts`，事件只 push、`onTick` 用 `activeAt(now)` 取 `progress`）；進度條 = `PlayerSprite` 的 `StatBar`（reversed）每 frame 依 `progress` 更新；飄傷害數字 = `EffectLayer.spawnDamage` | ✅ |
...
2. ~~Client 飄傷害數字 + 受擊表現~~——`EffectLayer` 已有 slash / fireball 動畫與 RO 式飄字。遺留：飄字的「大字」用傷害門檻判斷，未接 `crit` flag；`multipliers` 的屬性顏色未用。
3. MP 消耗驗證 ✅；冷卻仍為全域 `ATTACK_COOLDOWN_MS`，**per-skill 冷卻未做**（`world.attack` 留有 todo）。
4. ~~詠唱：`Skill.castTime` / server 計時 / `castBegin` / 到點結算廣播~~——已通，時鐘用簡單版（收到即起算）。
5. 打斷：~~移動取消詠唱（`moved`）~~；**`died` / `disconnected` 未做**——尚無死亡處理，斷線也不廣播取消。另待議：詠唱被打斷時 MP 已扣不退，規則未定。
6. 體感優化（本地預播動畫）——未做。
