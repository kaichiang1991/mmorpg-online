...
1. **Client 送意圖**：`attack { targetId }`。不含傷害值、不含命中判定。
2. **Server 驗證**：目標存在且活著？距離在射程內？冷卻結束？任一不過就靜默丟棄（與 `game.gateway.ts` 的 `onMove` payload 驗證同思路——非法輸入不回錯誤，直接忽略）。
3. **Server 結算**：呼叫已就位的 `CombatResolver.resolve(attacker, target, skill)`，產出 `Attack` 結算快照。
4. **廣播事件**：`attackResult { attackerId, targetId, damage, crit, kind, element }` 發給所有相關 client。A 與 B 收到同一份資料，畫面必然一致。
5. **HP 更新**：server 扣血後，新 HP 隨下一包 snapshot 自然到達所有 client，不需要事件裡重複帶。

### 3.3 體感優化（可選）

攻擊者本地可在按下攻擊時立刻播「揮刀動畫」（純視覺、無數字），等 `attackResult` 回來才飄傷害。動畫零延遲、數字仍權威。這是 client-side prediction 的最弱形式——預測的只有動畫，做壞了也不會造成狀態不同步。
...
```
skillCast { skillId, targetId }
  → server 驗證：會這技能? MP 夠? 冷卻? 距離?
  → 扣 MP（狀態，隨 snapshot 出去）
  → CombatResolver.resolve()
  → 廣播 skillResult { casterId, targetId, skillId, damage, crit, element, multipliers }
```

- MP 與 HP 相同，走 snapshot。
- `Attack.multipliers` 來源清單放進 event payload，client 可據此做戰報與飄字細節（暴擊字樣、屬性顏色），正好接上 combat-design.md 的決策三。
...
**二、完成由 server 判定。**
Client 進度條跑滿 ≠ 技能成功。Server 在 tick 中自行計時，到點才結算傷害並廣播 `castComplete`。Client 的進度條只是「預告」，真正的結果以 server 事件為準。
...
## 6. 協定草案（protocol.ts 增量）

> 僅示意形狀，實作時定案。欄位沿用 combat-design.md 的 `DamageKind` / `DamageElement` / `Multiplier`。
...
export interface ClientToServerEvents {
  ...
  castStart: (payload: CastStartPayload) => void;
}
...
export interface ServerToClientEvents {
  ...
  attackResult: (payload: AttackResultPayload) => void;
  castBegin: (payload: CastBeginPayload) => void;
  castComplete: (payload: AttackResultPayload) => void; // 完成即結算，同構
  castCancel: (payload: CastCancelPayload) => void;
}

/* ---- snapshot 增量：HP/MP 是狀態，走 snapshot ---- */
...
| 位置 | 改動 |
|------|------|
| `packages/shared/src/protocol.ts` | 上節事件與 payload 型別。共用型別 = 兩端編譯期就對齊，不會歪 |
| `game.gateway.ts` | 新增 `@SubscribeMessage('attack')`、`@SubscribeMessage('castStart')`，驗證 payload 形狀後轉 `GameService`；新事件廣播 |
...
| client `GameApp.ts` | 監聽 `attackResult`/`castBegin`/`castComplete`/`castCancel`；進度條 = `Graphics` 長方形每 frame 依 `progress` 更新寬度；飄傷害數字 |
| client `Interpolator` | `PlayerSnapshot` 加欄位後，HP/MP 直接取 `next` 值即可（不插值，血量跳變是可接受的） |

---

## 8. 建議實作順序

1. `protocol.ts` 加型別（純型別，零風險）。
2. `PlayerSnapshot` 加 `hp`/`maxHp`，server 端 `Player` 補欄位，client 畫血條——先讓「狀態同步」通。
3. `attack` 意圖 → 驗證 → `CombatResolver` → 廣播 `attackResult`——打通「事件同步」最短路徑，寫 gateway/service 測試。
4. Client 飄傷害數字 + 受擊表現。
5. 詠唱：`castStart` / `castBegin` / server 計時 / `castComplete`，時鐘先用簡單版（收到即起算）。
6. 打斷：移動取消詠唱、`castCancel` 廣播。
7. 冷卻、MP 消耗、體感優化（本地預播動畫）。
...
- **排除**：進度條逐 tick 廣播百分比——見第 5 節，時間戳方案全面優於它。
